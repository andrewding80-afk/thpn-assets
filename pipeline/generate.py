...
def run_job(job_path):
    ...
    print(f"[{name}] requesting {data['n']} variant(s) at {data['size']} bg={data['background']} ...")
    r = requests.post(ENDPOINT, headers={"Authorization": f"Bearer {API_KEY}"}, data=data, files=files, timeout=600)
